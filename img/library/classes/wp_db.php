...
class wpdb {
	...
	public function __construct( $dbuser, $dbpassword, $dbname, $dbhost ) {
		...
		// configure.php.php creation will manually connect when ready.
	...
	public function set_prefix( $prefix, $set_table_names = true ) {
		...
		if ( preg_match( '|[^a-z0-9_]|i', $prefix ) )
			die(__('Invalid database prefix'));
	...
	public function print_error( $str = '' ) {
		global $message_cls;
		if (isset($message_cls)) {
			$message_cls->set_error("mysql", $this->last_error);
		} else {
	...
	public function db_connect( $allow_bail = true ) {
		...
		if ( ! $this->dbh && $allow_bail ) {
			...
			$message .= '<p>' . sprintf(
				/* translators: 1: configure.php.php. 2: database host */
				__( 'This either means that the username and password information in your %1$s file is incorrect or we can&#8217;t contact the database server at %2$s. This could mean your host&#8217;s database server is down.' ),
				'<code>configure.php.php</code>',
	 ...
	 */
	public function get_row( $query = null, $output = WP_RESULT_TYPE, $y = 0 ) {
	 ...
	 */
	public function get_results( $query = null, $output = WP_RESULT_TYPE ) {
	...
	protected function get_table_charset( $table ) {
		 ...
		 */
		$charset = null; //apply_filters( 'pre_get_table_charset', null, $table );
		if ( null !== $charset ) {
			return $charset;
		}
		...
		if ( ! $results ) {
			die( 'wpdb_get_table_charset_failure' );
		}
...
//		}
	...
	protected function strip_invalid_text( $data ) {
		...
		if ( $db_check_string ) {
			...
			if ( ! $row ) {
				die (__( 'wpdb_strip_invalid_text_failure' ));
			}
	...
	public function check_database_version() {
		...
		if ( version_compare($this->db_version(), $required_mysql_version, '<') )
			die(__( '<strong>ERROR</strong>: WordPress %1$s requires MySQL %2$s or higher', $wp_version, $required_mysql_version ));
	}
