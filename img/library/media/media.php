 ...
 */
function image_resize( $file, $max_w, $max_h, $crop = false, $suffix = false, $dest_path = null, $jpeg_quality = 100 ) {

	$image = wp_load_image( $file );
	if ( !is_resource( $image ) )
	return show_error( 'error_loading_image', $image, $file );

	$size = @getimagesize( $file );
	if ( !$size )
	return show_error('invalid_image', ('Could not read image size'), $file);
	...
	if ( !$dims ) {
		...
		//		return show_error( 'error_getting_dimensions', ('Could not calculate resized image dimensions') );
	}
	...
	if ( IMAGETYPE_GIF == $orig_type ) {
		...
		return show_error('resize_path_invalid', ( 'Resize path invalid' ));
...
function image_crop( $file, $src_x, $src_y, $src_w, $src_h, $dest_path = null, $jpeg_quality = 100 ) {
	$image = wp_load_image( $file );
	if ( !is_resource( $image ) )
	return "error";
...
/* ]]> */
	 ...
	 *
	 * @param int $post_ID AD ID to delete the caches for.
	 */
	 ...
	 *
	 * @param int $post_ID AD ID to do the caching for.
	 */
...
function show_error($code = '', $message = '', $data = '') {
	echo $code . ">>" . $message;
}


#이미지를 비율에 관하여서만  리싸이즈 하는 함수
...
function img_resize ($img_src_file, $set_w, $set_h, $mode="s" )
{
	...
	} else if($mode == "s"){// 세팅 크기는 그대로 두되, 들어가는 이미지의 비율만 확대/축소한 다음 넣음
...
function img_download($img_url, $save_file) {
