...
function tep_not_null($value, $key = "", $str = "") {
	...
	if (is_array($value)) {
		if (sizeof($value) > 0) {
			...
			if ($key != '')
				$message_cls -> set_error($key, tep_get_message('ERROR_MUST_BE_NOT', $str));
			return false;
		}
	} else {
...
function tep_check_check($value, $check_value, $key = "", $str = "") {
	...
	if ($key != '')
		$message_cls -> set_error($key, tep_get_message('ERROR_MUST_BE_CHECK', $str));
	return false;
}
...
function tep_length_check($value, $min_len = 0, $max_len = 0, $key = '', $str = '') {
	...
	if (($min_len > 0 && strlen($value) < $min_len) || ($max_len > 0 && strlen($value) > $max_len)) {
		if ($key != '')
			$message_cls -> set_error($key, tep_get_message('ERROR_MUST_BE_LENGTH', $str, $min_len, $max_len));
...
function tep_number_check($value, $min = "", $max = "", $key = "", $str = "") {
	global $message_cls;
...
function tep_zipcode_check($coutry_code, $state_code, $cityname, $zipcode, $key = "", $str = "") {
	...
	$zipcount = $wpdb -> get_var("select count(*) as data_count from " . TABLE_CITIES_EXTENDED . " where zip='" . $zipcode . "' and city='" . $cityname . "' and state_code='" . $state_code . "' and country_code='" . $coutry_code . "'");
	...
	for ($i = 0; $i < count($valudataions); $i++) {
		...
		switch ($validation) {
			...
			case 'userid' :
				...
				break;
			case 'length' :
				tep_length_check($value, $parameter[0], count($parameter) == 2 ? $parameter[1] : 0, $key, $name);
				break;
			case 'email' :
			...
			case 'number' :
				if ($parameter == "") {
					tep_number_check($value, "", "", $key, $name);
				} else {
					tep_number_check($value, $parameter[0], $parameter[1], $key, $name);
				}
